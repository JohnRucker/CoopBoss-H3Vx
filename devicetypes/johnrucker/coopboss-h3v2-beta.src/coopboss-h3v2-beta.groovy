/**
 *  CoopBoss H3Vx  Buy hardware at CoopBoss.com
 *	03/10/16	Fixed Android Bug with state "default"
 *	01/26/16	Add new capabilities: Lock, Contact Sensor, Illuminance Measurement  
 ...
 */
metadata {
	definition (name: "CoopBoss H3V2-Beta", namespace: "JohnRucker", author: "John.Rucker@Solar-Current.com") {
        capability "Refresh"
        capability "Polling"
        capability "Sensor"
        capability "Actuator"
...
private Map parseReportAttributeMessage(String description) {
    ...
    if (descMap.cluster == "0101" && descMap.attrId == "0003") {
        ...
        sendEvent(name: "illuminance", value: (Integer.parseInt(descMap.value, 16)), displayed: false)
...
// Commands to device
def lock(){
	log.debug "lock calling closeDoor()"
    closeDoor()
}

def unlock(){
	log.debug "unlock calling openDoor()"
    openDoor()
